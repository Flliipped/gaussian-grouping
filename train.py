...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb):
    ...
    #        print("image:", image.shape)
        ...
        avg_surface_confidence = torch.tensor(0.0, device=image.device)
        avg_surface_axis_align = torch.tensor(0.0, device=image.device)
        avg_surface_axis_cosine = torch.tensor(0.0, device=image.device)
        avg_surface_boundary_score = torch.tensor(0.0, device=image.device)
        ...
        if iteration % 100 == 0 and (sugar_active or geo_active):
            ...
            if geo_active:
                print(
                    ...
                    + f"avg_surface_confidence={avg_surface_confidence.item():.6f}, "
                    + f"avg_surface_axis_align={avg_surface_axis_align.item():.6f}, "
                    + f"avg_surface_axis_cosine={avg_surface_axis_cosine.item():.6f}, "
                    + f"avg_surface_boundary_score={avg_surface_boundary_score.item():.6f}, "
    ...
    if use_wandb:
        ...
        if geo_active:
            log_data.update({
                ...
                "train_loss_patches/avg_surface_confidence": avg_surface_confidence.item(),
                "train_loss_patches/avg_surface_axis_align": avg_surface_axis_align.item(),
                "train_loss_patches/avg_surface_axis_cosine": avg_surface_axis_cosine.item(),
                "train_loss_patches/avg_surface_boundary_score": avg_surface_boundary_score.item(),
